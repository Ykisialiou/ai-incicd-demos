...
trivy fs --scanners vuln,secret,misconfig --format json --output "$SCAN_JSON" "$APP_DIR"
echo "✅ Real Trivy scan JSON generated at $SCAN_JSON ($(wc -c < "$SCAN_JSON") bytes)"

echo ""
echo "Step 1b: Raw scanner output, summarised (no interpretation)..."
echo "--------------------------------------------------------------------------------"
python3 "$SCRIPT_DIR/summarize_trivy.py" --input "$SCAN_JSON" --out "$SCRIPT_DIR/trivy_summary.md"
...
echo ""
echo "Step 2: Invoking Claude Code agent (claude CLI) to triage live findings..."
echo "--------------------------------------------------------------------------------"
...
echo ""
echo "================================================================================"
echo " ✅ Demo 2 complete! Live scan triaged by Claude AI agent."
echo "================================================================================"
